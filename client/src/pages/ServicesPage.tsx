  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-3">
        <h3 id={titleId} className="text-2xl font-bold uppercase text-navy-900">
          {service.name}
        </h3>
        {service.requiresApproval && <StatusBadge tone="info">Approval required</StatusBadge>}
      </div>
      {service.description && <p className="mt-1 text-sm text-navy-600">{service.description}</p>}

      <p className="mt-4 flex flex-wrap items-baseline gap-x-1.5">
        {priceVaries && <span className="text-sm text-navy-500">From</span>}
        <span className="text-2xl font-semibold text-navy-900">
          {shillings(service.fromPriceCents)}
        </span>
        <span className="text-sm text-navy-500">{PRICING_MODEL_UNIT[service.pricingModel]}</span>
      </p>

      <dl className="mt-4 space-y-3 text-sm">
        <div className="flex gap-2.5">
          <dt className="pt-0.5 text-navy-400">
              ...
              {service.rooms.map((room) => (
                <li key={room.slug} className="flex items-baseline justify-between gap-3">
                  <span className="text-navy-700">
                    {room.name}
                    <span className="text-navy-400"> · up to {room.capacity}</span>
                  </span>
                  {roomPricesDiffer && (
      ...
      {service.packages.length > 0 && (
        <div className="mt-4 border-t border-navy-100 pt-4">
          <h4 className="text-xs font-semibold uppercase tracking-wide text-navy-500">Packages</h4>
      ...
      <div className="mt-auto pt-5">
        <Link
          to="/book"
          className="inline-flex h-10 w-full items-center justify-center rounded-lg bg-signal-600 px-4 text-sm font-semibold text-white transition-colors hover:bg-signal-700"
        >
          Book this service
        </Link>
      </div>
